...
export function recommendMainPlaces(userPrefs, places, topN = 5, savedPlaces = []) {
  ...
  if (hgPref === "hidden gems") {
    candidatePlaces = candidatePlaces.filter((p) => (p.popularity_norm || "").toLowerCase() !== "high");
  }
  ...
  const scored = candidatePlaces.map((place) => {
    ...
    const isResident = userPrefs.user_type === 'resident';
    const wPref = 0.50;
    const wRat = isResident ? 0.05 : 0.20;
    const wHg = isResident ? 0.30 : 0.15;
    const wLoc = 0.15;
...
export function recommendNearbyPlaces(selectedPlace, userPrefs, places, topN = 5) {
  ...
  let candidatePlaces = places.filter((p) => p.place_name !== selName);
  if (hgPref === "hidden gems") {
    candidatePlaces = candidatePlaces.filter((p) => (p.popularity_norm || "").toLowerCase() !== "high");
  }
  ...
  const scored = candidatePlaces.map((place) => {
      ...
      const wPref = 0.30;
      const wRat = isResident ? 0.05 : 0.20;
      const wHg = isResident ? 0.25 : 0.10;

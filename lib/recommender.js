...
export function calcLocationScore(userPrefs, place) {
  ...
  if (!pref || pref === "any") return 65;

  const province = (place.province_or_city || "").toLowerCase();
  const district = (place.district || "").toLowerCase();

  if (province === pref) return 100;
  if (province.includes(pref) || pref.includes(province.split(" ")[0])) return 90;
  if (district.includes(pref.split(" ")[0])) return 70;
  return 15;
}

// ─── Stage 1: Main personalized recommendations ──────────────────────────────

export function recommendMainPlaces(userPrefs, places, topN = 5) {
  const scored = places.map((place) => {
    ...
    // No random noise — deterministic, sorted by real scoring
  ...
  return scored
    .filter((p) => p._prefS > 10) // must have meaningful preference match
